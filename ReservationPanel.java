...
public class ReservationPanel extends ViewPanels{
	...
	private void display(Iterator allTasks) {
		int j = 0;
		if (!allTasks.hasNext()) {
			modelViewTable.setValueAt("No Reservations", 0, 1);
		} else {
			for (Iterator it = allTasks; it.hasNext();) {
				Task t = (Task)it.next();
				String tskName = t.getName();
				String tskTime = t.getDayNum() + "/" + t.getMonth() + "/" + t.getYear() + " " + 
								t.getStrStartTime() + " - " + t.getStrEndTime();
	...
	private void cancelAppointment() {
		...
		Task t = null;
		for (int i = 0; i < row; i++) {
			t = (Task) allTasks.next();
		}
		controller.cancelAppointment(frameTitle, t);
	}
